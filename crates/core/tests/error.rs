...
fn variants_format_human_readable() {
    let cases: Vec<(SessionError, &str)> = vec![
        (
            SessionError::NoBinary(PathBuf::from("/nope/claude")),
            "claude binary not found at /nope/claude",
        ),
        (
            SessionError::UnexpectedExit(137),
            "claude exited unexpectedly: code 137",
        ),
        (
            SessionError::ControlTimeout(Duration::from_millis(500)),
            "control request timed out after 500ms",
        ),
        (
            SessionError::Lagged,
            "ipc channel lagged; client must resync",
        ),
    ];
